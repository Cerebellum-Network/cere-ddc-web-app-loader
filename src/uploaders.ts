...
const buildDdcFileUrl = async (ddcClient: DdcClient, bucketId: bigint, appId: string,
...
export const uploadWebApp = async (ddcClient: DdcClient,
  ...
  await Promise.all(files.map(async (file) => {
    ...
    if (file === indexFilePath) {
      mainRoute = rewrites[indexFileName] || response.url;
    } else {
      const rewritePath = rewrites[relativePath] || relativePath
      routes[relativePath] = await buildDdcFileUrl(ddcClient, bucketId, applicationPath, rewritePath);
    }
  ...
  const swAppId = rewrites['sw.js'] ? path.join(applicationPath, rewrites['sw.js']) : `${applicationPath}-sw.js`;
  const swTags = [
  ...
  const loaderHtml = appTmpl({
    sw: rewrites['sw.js'] ? rewrites['sw.js'] : swRoute,
    mainRoute,
    routes,
  });
